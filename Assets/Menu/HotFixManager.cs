...
namespace Hotfix
{
    ...
    {
        #region 需要配置项目
        public static List<string> configs = Resources.Load<TextAsset>("HotFix").text.Split("\n").ToList();
        public static string ProjectName { get; } = configs[1];
        public static string ServerIP { get; } = configs[3];

        //static string serverDownloadUrl = $"http://106.15.38.165:495/Download";
        public static string AB_UploadUrl { get; } = $"{ServerIP}/AB_Upload/{ProjectName}";
        #endregion
        #region UI
        ...
        private async Task CheckAssetBundles()
        {
            ...
            {
                var responseMessage = await httpClient.GetAsync($"{AB_UploadUrl}/{ServerTag}/MD5.json");
                    ...
                    else
                    {
                        ...
                        async Task DownLoadFile(KeyValuePair<string, byte[]> MD5FiIeData, FileInfo localFile)
                        {
                            ...
                            using (WebClient webClient = new WebClient())
                            {
                                ...
                                Debug.LogWarning("下载文件" + $"{AB_UploadUrl}/{ServerTag}/{MD5FiIeData.Key}");
                                await webClient.DownloadFileTaskAsync(new System.Uri($"{AB_UploadUrl}/{ServerTag}/{MD5FiIeData.Key}"), localFile.FullName);
                ...
                //指定热更场景和资源网络路径
                onlineDllOrApkPath = $"{AB_UploadUrl}/{ServerTag}_Dll/TouHouMachineLearningSummary.dll";
                onlineDllOrApk_MD5Path = $"{AB_UploadUrl}/{ServerTag}_Dll/MD5.json";
