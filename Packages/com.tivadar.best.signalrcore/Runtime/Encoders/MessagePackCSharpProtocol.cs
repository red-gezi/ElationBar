...
namespace Best.SignalR.Encoders
{
    ...
    public sealed class MessagePackCSharpProtocol : Best.SignalR.IProtocol
    {
        ...
        public void ParseMessages(BufferSegment segment, ref List<Message> messages)
        {
            ...
            while (offset < segment.Count)
            {
                ...
                switch ((MessageTypes)messageType)
                {
                    ...
                    case MessageTypes.Ack: messages.Add(ReadAck(ref reader)); break;
                    case MessageTypes.Sequence: messages.Add(ReadSequence(ref reader)); break;
                }
        ...
        private Message ReadSequence(ref MessagePackReader reader)
        {
            long sequenceId = reader.ReadInt64();

            return new Message
            {
                type = MessageTypes.Sequence,
                sequenceId = sequenceId,
            };
        }

        private Message ReadAck(ref MessagePackReader reader)
        {
            long sequenceId = reader.ReadInt64();
