...
namespace Best.SignalR.Encoders
{
    ...
    /// <summary>
    /// <see cref="Best.SignalR.IProtocol"/> implementation using the <see href="Json &amp; MessagePack Serialization">https://assetstore.unity.com/packages/tools/network/json-messagepack-serialization-59918</see> asset store package.
    /// </summary>
        ...
        public BufferSegment EncodeMessage(Message message)
        {
            ...
            switch (message.type)
            {
                ...
                case MessageTypes.Ack:
                    // https://github.com/dotnet/aspnetcore/blob/main/src/SignalR/docs/specs/HubProtocol.md#ack-message-encoding-1
                    // [8, SequenceId]

                    writer.WriteArrayBegin(2);
                    writer.WriteNumber(8);
                    writer.WriteNumber(message.sequenceId);
                    writer.WriteArrayEnd();
                    break;

                case MessageTypes.Sequence:
                    // https://github.com/dotnet/aspnetcore/blob/main/src/SignalR/docs/specs/HubProtocol.md#sequence-message-encoding-1
                    // [9, SequenceId]
                ...
                using (var stream = new System.IO.MemoryStream(segment.Data, offset, length))
                {
                    ...
                    try
                    {
                        ...
                        switch ((MessageTypes)messageType)
                        {
                            ...
                            case MessageTypes.Ack: 
                                messages.Add(new Message { type = MessageTypes.Ack, sequenceId = reader.ReadInt64()}); 
                                break;
                            case MessageTypes.Sequence: 
                                messages.Add(new Message { type = MessageTypes.Sequence, sequenceId = reader.ReadInt64() }); 
                                break;
                        }
