...
namespace Best.SignalR.Messages
{
    ...
    public sealed class NegotiationResult
    {
        ...
        /// <summary>
        /// If set to true, the connection should attempt to reconnect with the same <see cref="ConnectionId"/> if it disconnects.
        /// </summary>
        public bool UseStatefulReconnect { get; private set; }
        ...
        internal static NegotiationResult Parse(HTTP.HTTPResponse resp, out string error, HubConnection hub)
        {
            ...
            try
            {
                ...
                result.UseStatefulReconnect = response.TryGetValue("useStatefulReconnect", out value) && bool.TryParse(value?.ToString(), out var useACK) && useACK;
