...
using Best.SignalR.Examples.Helpers;
...
using TaskCompletionSource = Cysharp.Threading.Tasks.UniTaskCompletionSource;
#endif


namespace Best.SignalR.Examples
{
    ...
    class TestHubSample : Best.SignalR.Examples.Helpers.SampleBase
    {
...
#pragma warning restore
        ...
        public void OnConnectButton()
        {
...
#elif _BESTHTTP_SIGNALR_CORE_ENABLE_GAMEDEVWARE_MESSAGEPACK
            ...
            //hub.AuthenticationProvider = new Best.SignalRCore.Authentication.HeaderAuthenticator("<generated jwt token goes here>");
        ...
        private void Hub_OnConnected(HubConnection hub)
        {
            ...
            // Function callback
#if WITH_UNITASK
            hub.On<string, int, int, UniTask<int>>("GetResult", (string description, int min, int max) =>
            {
                ...
                return Task.FromResult(UnityEngine.Random.Range(min, max));
            });

            // Final result called by the server
            hub.On<string>("EndResult", async (result) =>
            {
                UnityEngine.Debug.Log("EndResult!");
                await Task.Delay(10_000);
                UnityEngine.Debug.Log(result);
            });
#else
            hub.On<string, int, int, Task<int>>("GetResult", (string description, int min, int max) =>
            {
                UnityEngine.Debug.Log(description);
